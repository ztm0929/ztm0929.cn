...
// 暂时不可用标记，接入外部 API 后将此值改为 false
const FEATURE_UNAVAILABLE = true;
...
export function AISearchInput(props: ComponentProps<'form'>) {
  ...
  return (
    <form {...props} className={cn('flex items-start pe-2', props.className)} onSubmit={onStart}>
      <Input
        value={input}
        placeholder={FEATURE_UNAVAILABLE ? '功能暂未开放' : isLoading ? 'AI is answering...' : 'Ask a question'}
        autoFocus
        className="p-3"
        disabled={FEATURE_UNAVAILABLE || status === 'streaming' || status === 'submitted'}
        onChange={(e) => {
      ...
      <Presence present={open}>
        <div
          className={cn(
            ...
            'lg:fixed lg:right-0 lg:top-0 lg:h-dvh lg:border-s',
            open
    ...
    >
      {messages.length === 0 ? (
        <div className="text-sm text-fd-muted-foreground/80 size-full flex flex-col items-center justify-center text-center gap-2">
          ...
          {FEATURE_UNAVAILABLE ? (
            <p onClick={(e) => e.stopPropagation()}>AI 对话功能暂未开放，敬请期待。</p>
          ) : (
